...
In order to run this script, the [`raw_GDSC2_data.zip` file](https://drive.google.com/open?id=1nsfVgA-3AQFr6khf80FW2b_d2hYR_7jO) must be downloaded from Google Drive. 
It should be a directory within the `Data` directory of this R Project.
The original dataset is documented at [www.cancerrxgene.org](https://www.cancerrxgene.org). 
...
```{r read_gdsc_meta}
cell_line_details <- read.csv("../Data/raw_GDSC2_Data/cell_line_details.csv", 
...
```{r upload_gdsc_drug}
dr_gdsc2 <- read.csv2("../Data/raw_GDSC2_Data/GDSC2_fitted_dose_response_17Jul19.csv", 
...
```{r read_gdsc_expr}
expr_gdsc_data <- read.csv2("../Data/raw_GDSC2_Data/Cell_line_RMA_proc_basalExp.csv", 
                            sep = ",", stringsAsFactors = FALSE)
...
gdsc_hgnc_entrez <- readRDS("../biomaRt_query_091619.rds")
```
...
save(dr_gdsc2, 
     expr_gdsc_data, 
     cell_line_details, 
     file = "../Data/cleaned_gdsc_data.RData")
# load("../Data/cleaned_gdsc_data.RData")
```
